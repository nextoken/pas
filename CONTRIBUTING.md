...
5. **Path Handling**: Use `pathlib.Path` for all filesystem operations. Expand users (`~`) and resolve paths to handle drag-and-drop. For typed or pasted shell-quoted paths (e.g. `"/path/to/file"`), call `normalize_path_input` from `helpers/core.py` before building `Path(...)`.
